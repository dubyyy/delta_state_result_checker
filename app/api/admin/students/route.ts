...
export async function GET(request: NextRequest) {
  try {
    ...
    // If exporting, set a very high limit or disable pagination logic where possible
    // For simplicity, if export is true, we can set a high limit, but since we have slice logic later, we need to handle it carefully.
    // Let's keep limit calculation but override it if export is true.
    const limitParam = parseInt(searchParams.get("limit") || "100");
    const limit = isExport ? 1000000 : Math.min(limitParam, 1000); // Max 1000 per page normally, huge for export
      ...
      const [regularStudents, regularCount] = await Promise.all([
        prisma.studentRegistration.findMany({
          ...
          select: {
            ...
            religiousType: true, religiousTerm1: true, religiousTerm2: true, religiousTerm3: true,
            createdAt: true,
            prcd: true,
            year: true,
