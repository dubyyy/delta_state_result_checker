...
const SchoolRegistration = () => {
  ...
  const registerPostStudentFast = async (registration: Registration, skipDuplicate = false): Promise<{ isDuplicate: boolean; duplicate?: any; registrations?: any[] }> => {
    ...
    const response = await fetch('/api/school/register-post-student', {
      method: 'POST',
      headers: {
  ...
  const processRegistration = async (newRegistration: Registration) => {
    try {
      const result = await registerPostStudentFast(newRegistration);
      ...
      if (result.registrations) {
        ...
        // API failed but no duplicate — error was already set by registerPostStudentFast,
            ...
            <AlertDialogAction
              onClick={async () => {
                if (pendingRegistration) {
                  try {
                    const result = await registerPostStudentFast(pendingRegistration, true);
                    if (result.registrations) {
                      const mapped: Registration[] = result.registrations.map((r: any) => ({
                        id: r.id,
                        studentNumber: r.studentNumber,
