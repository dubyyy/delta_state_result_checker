...
const SchoolRegistration = () => {
  ...
  // Unified fast registration: duplicate check + student number + save in ONE API call
  const registerStudentFast = async (registration: Registration): Promise<{ isDuplicate: boolean; duplicate?: any; registrations?: any[] }> => {
    ...
    const response = await fetch('/api/school/register-student', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        ...
        general: registration.general,
        religious: registration.religious,
        isLateRegistration: registration.isLateRegistration,
        year: registration.year,
        prcd: registration.prcd,
      }),
    });
    ...
    if (!response.ok && response.status !== 200) {
      setSaveMessage({ type: 'error', text: data.error || 'Failed to register student' });
      return { isDuplicate: false };
    }
  ...
  const loadRegistrationsFromServer = async () => {
    try {
      ...
      const mapped: Registration[] = (data.registrations || []).map((r: any) => ({
        ...
        dateOfBirth: r.dateOfBirth ? isoToDdmmyyyy(r.dateOfBirth.split('T')[0]) : '',
        gender: r.gender,
  ...
  const handlePrint = () => {
    ...
    const headerLine3 = `${academicYear} COGNITIVE/PLACEMENT EXAMINATION FOR PRIMARY SIX PUPILS`;
  ...
  // Generate student number: xfffNNNN format where NNNN is based on alphabetical rank + count
  const generateStudentNumber = (
    ...
    const targetSurname = normalize(surname);
    const targetFirstname = normalize(firstname);

    // Build a set of unique, normalized surnames
    const allSurnames = currentRegs.map((r) => normalize(r.lastname));
    const uniqueSurnames = Array.from(new Set([...allSurnames, targetSurname])).sort((a, b) => a.localeCompare(b));

    // Get the alphabetical rank of this surname (1-based)
    const surnameRank = uniqueSurnames.indexOf(targetSurname) + 1;

    // Count how many students with the same surname have a first name that comes before this one alphabetically
    // Plus 1 for the current student
    const studentsWithSameSurname = currentRegs.filter(r => normalize(r.lastname) === targetSurname);
    const surnameCount = studentsWithSameSurname.filter(r => normalize(r.firstname) < targetFirstname).length + 1;

    // Use sequential numbering within each surname, maintaining alphabetical order
    // Adams students get 0001, 0002, etc., Smith students get 0001, 0002, etc.
    // But overall alphabetical order determines the actual numbers
    const allStudents = [...currentRegs, { lastname: surname, firstname: firstname } as Registration]
      .sort((a, b) => {
    ...
    // Find the position of this student in the fully sorted list
    const studentPosition = allStudents.findIndex(
      r => normalize(r.lastname) === targetSurname && normalize(r.firstname) === targetFirstname
    ) + 1;
    
    const uniqueNumber = studentPosition;

    const x = school.lgaCode; // 1-2 digits
    const fff = school.schCode.padStart(3, '0'); // 3 digits (padded)
    const nnnn = uniqueNumber.toString().padStart(4, '0'); // 4 digits from alphabetical rank
  ...
  // Process registration using the unified fast endpoint (1 API call instead of 4)
  const processRegistration = async (newRegistration: Registration) => {
    setIsRegistering(true);
    try {
      const result = await registerStudentFast(newRegistration);

      if (result.isDuplicate) {
        // Server found a duplicate — show dialog
        setDuplicateNames([result.duplicate]);
        setPendingRegistration(newRegistration);
        setShowDuplicateDialog(true);
        return;
      }

      // Success — update UI from server response (no extra fetch needed)
      if (result.registrations) {
        const mapped: Registration[] = result.registrations.map((r: any) => ({
          id: r.id,
          studentNumber: r.studentNumber,
          lastname: r.lastname,
          othername: r.othername || '',
        ...
        setSaveMessage({ type: 'success', text: 'Student registered successfully!' });
        setTimeout(() => setSaveMessage(null), 5000);
        ...
        // API failed but no duplicate — error was already set by registerStudentFast,
        // but if somehow it wasn't, set a fallback error
        if (!saveMessage) {
          setSaveMessage({ type: 'error', text: 'Failed to register student. Please try again.' });
        }
        return;
      }
    } catch (error) {
      console.error('Registration error:', error);
      setSaveMessage({ type: 'error', text: 'An error occurred while registering. Please try again.' });
    } finally {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      {showSuccessNotification && (
        <div className="fixed top-6 left-1/2 transform -translate-x-1/2 z-50 animate-in fade-in slide-in-from-top-4 duration-500">
          <div className="bg-white border-l-4 border-green-600 px-6 py-4 rounded-md shadow-lg flex items-start gap-4 min-w-[380px] max-w-md">
            <div className="flex-shrink-0 w-10 h-10 bg-green-100 rounded-full flex items-center justify-center">
              <CheckCircle2 className="h-6 w-6 text-green-600" />
            </div>
            <div className="flex-1">
              <p className="font-semibold text-gray-900 text-base">Student Registered Successfully</p>
              <p className="text-sm text-gray-600 mt-1">The student has been added to the database</p>
            </div>
            <button 
      ...
      {isRegistering && (
        <div className="fixed inset-0 bg-black/50 z-40 flex items-center justify-center">
          <div className="bg-white rounded-lg p-8 shadow-2xl flex flex-col items-center gap-4">
            ...
            <p className="text-xl font-semibold text-gray-900">Registering Student...</p>
            <p className="text-sm text-gray-600">Please wait while we process the registration</p>
          </div>
        </div>
      )}
        ...
        <div className="max-w-2xl xl:max-w-3xl 2xl:max-w-4xl mx-auto">
              ...
              {!isLoggedIn ? (
                ...
                <>
                  ...
                  <form className="space-y-6" onSubmit={async (e) => {
                    ...
                    // Unified: duplicate check + save happens in one API call
          ...
          {isLoggedIn && registrations.length > 0 && (
            <Card className="mt-8">
              ...
              <CardContent>
                {saveMessage && (
                  <div className={`mb-6 overflow-hidden rounded-xl transition-all duration-500 ease-in-out animate-in fade-in slide-in-from-top-4 ${saveMessage.type === 'success'
                    ...
                    <div className="relative p-6">
                      ...
                      <div className="absolute inset-0 opacity-10">
                        <div className={`absolute top-0 right-0 w-32 h-32 rounded-full blur-3xl ${saveMessage.type === 'success' ? 'bg-emerald-400' : 'bg-red-400'
                          }`} />
                        <div className={`absolute bottom-0 left-0 w-24 h-24 rounded-full blur-2xl ${saveMessage.type === 'success' ? 'bg-teal-400' : 'bg-rose-400'
                          }`} />
                      </div>
                      ...
                      <div className="relative flex items-start gap-4">
                        ...
                        <div className={`shrink-0 relative ${saveMessage.type === 'success' ? 'text-emerald-600' : 'text-red-600'
                          }`}>
                          <div className={`absolute inset-0 rounded-full blur-md opacity-40 animate-pulse ${saveMessage.type === 'success' ? 'bg-emerald-400' : 'bg-red-400'
                            }`} />
                          <div className={`relative flex items-center justify-center w-12 h-12 rounded-full ${saveMessage.type === 'success'
                        ...
                        <div className="flex-1 min-w-0">
                          <h3 className={`font-bold text-xl mb-2 tracking-tight ${saveMessage.type === 'success' ? 'text-emerald-900' : 'text-red-900'
                            }`}>
                            {saveMessage.type === 'success' ? '✨ Successfully Saved!' : '⚠️ Error Occurred'}
                          </h3>
                          <p className={`text-base leading-relaxed ${saveMessage.type === 'success' ? 'text-emerald-800/90' : 'text-red-800/90'
                            }`}>
                            {saveMessage.text}
                          </p>
                          {saveMessage.type === 'success' && (
                        ...
                        <button
                          ...
                          className={`shrink-0 rounded-lg p-2 transition-all duration-200 hover:scale-110 ${saveMessage.type === 'success'
                              ...
                              <TableCell>
                                {isEditing ? (
                                  ...
                                  <div className="font-medium">
                                    {[reg.lastname, reg.othername, reg.firstname].filter(Boolean).join(' ')}
                                  </div>
                                )}
                              </TableCell>
                              <TableCell>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="space-y-4 pt-2">
              <p className="text-center text-gray-600">
                A student with the exact same name is already registered:
              </p>
              ...
              <p className="text-center text-sm text-gray-500 italic">
                This appears to be the exact same student. Register anyway?
              </p>
              <p className="text-center text-xs text-gray-400">
                Note: Brothers with different first names can register without this warning.
              </p>
            </AlertDialogDescription>
          </AlertDialogHeader>
                  ...
                  try {
                    ...
                    const response = await fetch('/api/school/register-student', {
                      ...
                      body: JSON.stringify({
                        ...
                        othername: pendingRegistration.othername,
                        dateOfBirth: pendingRegistration.dateOfBirth,
                        gender: pendingRegistration.gender,
                        schoolType: pendingRegistration.schoolType,
                        passport: pendingRegistration.passport,
                        english: pendingRegistration.english,
                        arithmetic: pendingRegistration.arithmetic,
                        general: pendingRegistration.general,
                        religious: pendingRegistration.religious,
                        isLateRegistration: pendingRegistration.isLateRegistration,
                    ...
                    if (response.ok || response.status === 201) {
                      ...
                      // Reset form fields
                      setSelectedImage(null); setGender(""); setSchoolType(""); setReligiousType("");
                      setLastname(""); setFirstname(""); setOthername(""); setDateOfBirth("");
                      setEnglishYear1(""); setEnglishYear2(""); setEnglishYear3("");
                      setArithmeticYear1(""); setArithmeticYear2(""); setArithmeticYear3("");
                      setGeneralYear1(""); setGeneralYear2(""); setGeneralYear3("");
                      setReligiousYear1(""); setReligiousYear2(""); setReligiousYear3("");
                    } else {
                      setSaveMessage({ type: 'error', text: data.error || 'Failed to register student' });
                    }
                  } catch (error) {
